...
template <typename K, typename V> class RedBlackTreeIterator {
public:
  /**
   * Constructor
   */
   ...
   * by node_ptr.
   */
  Node<K, V> &operator*() const { return *node_ptr; }
...
template <typename K, typename V> class RedBlackTree : public Tree {
public:
   ...
   * Destructor.
   */
  ~RedBlackTree() { delete_tree(root_); }
   ...
   */
  size_t leaf_count() const { return leaf_count(root_); }

  /**
   * Returns the internal node count of the red-black tree.
   */
  size_t internal_node_count() const { return internal_node_count(root_); }
...
private:
  ...
  int height(Node<K, V> *node) const {
    ...
    if (node == nullptr) {
      ...
      return (1 + std::max(height_right, height_left));
    }
  }
  ...
  size_t leaf_count(Node<K, V> *node) const {
    ...
    if (node == nullptr) {
      ...
      if ((node->left == nullptr) && (node->right == nullptr)) {
        ...
        int left_count = leaf_count(node->left);
        int right_count = leaf_count(node->right);
  ...
  size_t internal_node_count(Node<K, V> *node) const {
    ...
    if ((node->left == nullptr) && (node->right == nullptr)) {
      ...
      int left_count = internal_node_count(node->left);
      int right_count = internal_node_count(node->right);
  ...
  int diameter(Node<K, V> *node) const {
    ...
    if (node == nullptr) {
      ...
      int height_left = height(node->left);
      int height_right = height(node->right);
      int diameter_left = diameter(node->left);
      int diameter_right = diameter(node->right);
      return (std::max((height_left + height_right + 2), std::max(diameter_left, diameter_right)));
    }
  }
  ...
  size_t width(Node<K, V> *node, size_t level) const {
    ...
    if (level == 0) {
      ...
      int width_left = width(node->left, level - 1);
      int width_right = width(node->right, level - 1);
      return width_left + width_right;
    }
  }

  size_t null_count() const { return null_count(root_); }
  ...
  size_t null_count(Node<K, V> *node) const {
    ...
    if (node == nullptr) {
      ...
      int null_count_left = null_count(node->left);
      int null_count_right = null_count(node->right);
      return (null_count_left + null_count_right);
    }
  }

  size_t sum_levels() const { return sum_levels(root_, 0); }
  ...
  size_t sum_levels(Node<K, V> *node, size_t level) const {
    ...
    if (node == nullptr) {
      ...
      int sum_levels_left = sum_levels(node->left, level + 1);
      int sum_levels_right = sum_levels(node->right, level + 1);
      return (level + sum_levels_left + sum_levels_right);
    }
  }

  size_t sum_null_levels() const { return sum_null_levels(root_, 0); }
  ...
  size_t sum_null_levels(Node<K, V> *node, size_t level) const {
    ...
    if (node == nullptr) {
      ...
      int sum_levels_left = sum_null_levels(node->left, level + 1);
      int sum_levels_right = sum_null_levels(node->right, level + 1);
      return (sum_levels_left + sum_levels_right);
    }
  }
};
